...
static int snd_card_saudio_pcm_open(struct snd_pcm_substream *substream)
{
	...
	int result = 0;
	struct saudio_dev_ctrl *dev_ctrl = NULL;
	struct mutex saudio_mutex = saudio->mutex;
	ADEBUG();

	mutex_lock(&saudio_mutex);
	if(!saudio->state) {
		mutex_unlock(&saudio_mutex);
...
static void saudio_work_card_free_handler(struct work_struct *data)
{
	...
	if (saudio->card) {
		...
		printk(KERN_INFO
		       "saudio: work_handler:snd card free result %d, dst %d, channel %d\n",
		       result, saudio->dst, saudio->channel);
	}
...
static int snd_saudio_probe(struct platform_device *devptr)
{
	...
	if(mRecovery) {
		printk("saudio: Recovery mode, saudio Skipped\n");
		return -1;
	}
...
{
	if((0 == strcmp(str, "2"))
		|| (0 == strcmp(str, "aaa ap_only_boot"))
		|| (0 == strcmp(str, "aaa"))) {
		mRecovery = true;
	} else {
		mRecovery = false;
	}
	printk("saudio: bootmode : %s\n", str);
	printk("saudio: mRecovery : %d\n", mRecovery);
	return 1;
 }
