...
//Bugzilla/SPRD – Bug 318775
static ssize_t i2c_show(struct device* cd, struct device_attribute *attr,
                const char* buf)
{
	printk("%s\n",__func__);
	return snprintf(buf, sizeof(char) * 16, "i2c_show\n");
}

static DEVICE_ATTR(reset, S_IRUGO | S_IWUSR, i2c_show, i2c_reset);
...
static int sprd_i2c_probe(struct platform_device *pdev)
{
	...
	ret = i2c_create_sysfs(pdev);
	if (ret) {
		printk("I2C:i2c_create_sysfs failed!\n");
		goto err_sysfs;
	}
