...
static void __cpuinit sprd_plugin_one_cpu_ss(struct work_struct *work)
{
	...
	sd_tuners->cpu_num_limit = min((unsigned int)pm_qos_request(PM_QOS_CPU_ONLINE_MAX), sd_tuners->cpu_num_limit);
...
static void sprd_unplug_one_cpu_ss()
{
	...
	struct sd_dbs_tuners *sd_tuners = g_sd_tuners;
	sd_tuners->cpu_num_limit = max((unsigned int)pm_qos_request(PM_QOS_CPU_ONLINE_MIN), sd_tuners->cpu_num_limit);
#ifdef CONFIG_HOTPLUG_CPU
	if (num_online_cpus() > sd_tuners->cpu_num_limit) {
		if (!g_sd_tuners->cpu_hotplug_disable) {
...
#if 0 /* disable cpu hotplug default value, we enable from power init */
