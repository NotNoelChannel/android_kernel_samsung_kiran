...
void gpio_mon_save_gpio_state(unsigned char phonestate, unsigned int gpio_no, \
	...
	if(phonestate==PHONE_INIT)
		{
		gpio_mon_gpio_init_state_map[gpio_no].pin_name_sel = PIN_NAME_sel ;
		...
		gpio_mon_gpio_sleep_state_map[gpio_no].pin_name_sel = PIN_NAME_sel ;
...
unsigned int i=0;
	...
	for(i = 0 ; i < gpio_mon_gpio_count ; i ++)
	{
		...
		size += snprintf(buf+size,GPIO_MON_BUF_SIZE - size, \
			"  %5d               %-5s              %-5s             %-5s                %-5s                \
			%-5s           %-5s\n",\
			i, direction_i, resistor_i, state_i, direction_s, resistor_s, state_s);
		}
