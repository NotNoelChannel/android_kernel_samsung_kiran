...
#elif CONFIG_PARTIALSUSPEND_SLP
static void devfreq_pre_suspend(struct pre_suspend *h)
{
...
static void devfreq_post_resume(struct pre_suspend *h)
{
	dfs_set_freq(0);
}

static struct pre_suspend devfreq_pre_suspend_desc = {
        .level = PRE_SUSPEND_LEVEL_DISABLE_FB + 100,
        .suspend = devfreq_pre_suspend,
        .resume = devfreq_post_resume,
};
#endif
...
#elif CONFIG_PARTIALSUSPEND_SLP
static void devfreq_enable_post_resume(struct pre_suspend *h)
{
	gov_eb = 1;
}

static struct pre_suspend devfreq_enable_desc = {
        .level = PRE_SUSPEND_LEVEL_BLANK_SCREEN,
        .resume = devfreq_enable_post_resume,
};
#endif
/************ userspace interface *****************/
...
static int devfreq_ondemand_start(struct devfreq *devfreq)
{
	...
	#elif CONFIG_PARTIALSUSPEND_SLP
	register_pre_suspend(&devfreq_pre_suspend_desc);
	#endif
	/*
	...
	*/
	...
	#elif CONFIG_PARTIALSUSPEND_SLP
	register_pre_suspend(&devfreq_enable_desc);
	#endif
