				      ...
				      struct devfreq_dev_status *stat)
{
	...
	total_bw = (stat->current_frequency);
	...
	*/
	if(interval){
		stat->busy_time = (u32) trans_bw * HZ /1000;
		stat->total_time = total_bw * interval;
	}else{
...
#if defined(CONFIG_SYSTEM_LOAD_ANALYZER)
{
	...
	if (total_time == 0)
		total_time = 1;
	store_external_load_factor(MIF_BUS_LOAD, stat->busy_time*100 /total_time );
