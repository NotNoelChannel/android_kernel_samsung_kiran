...
#elif CONFIG_PARTIALSUSPEND_SLP
static void sprdemand_gov_pre_suspend(struct pre_suspend *h)
{
        pr_info("%s do nothing\n", __func__);
        return;
}

static void sprdemand_gov_post_resume(struct pre_suspend *h)
{
        struct cpufreq_policy *policy = cpufreq_cpu_get(0);
        struct dbs_data *dbs_data = policy->governor_data;
        struct sd_dbs_tuners *sd_tuners = NULL;
...
static struct pre_suspend sprdemand_gov_presuspend_handler = {
        .level = PRE_SUSPEND_LEVEL_BLANK_SCREEN,
        .suspend = sprdemand_gov_pre_suspend,
        .resume = sprdemand_gov_post_resume,
};
#endif
...
#elif CONFIG_PARTIALSUSPEND_SLP
	register_pre_suspend(&sprdemand_gov_presuspend_handler);
#endif
...
#elif CONFIG_PARTIALSUSPEND_SLP
	unregister_pre_suspend(&sprdemand_gov_presuspend_handler);
#endif
