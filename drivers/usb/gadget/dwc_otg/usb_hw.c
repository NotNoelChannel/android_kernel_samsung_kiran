	...
	if( GPIO_INVALID !=ldo_gpio)
	   {
		...
		struct regulator *usb_regulator = regulator_get(NULL,USB_LDO_NAME);
		...
		if(!IS_ERR_OR_NULL(usb_regulator)){
			if(is_on){
				...
				if(ret != 0){
					pr_info("%s: regulator_enable() failed: %d\n", __func__ ,ret);
				}
			}else{
...
void usb_phy_init(struct platform_device *_dev)
{
	...
	__raw_writel(tune_from_uboot,(void *)REG_AP_APB_USB_PHY_TUNE);
