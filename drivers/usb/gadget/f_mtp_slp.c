...
static struct usb_request *mtpg_request_new(struct usb_ep *ep, int buffer_size)
{
	...
	/* now allocate buffers for the requests
	  * Fix for PLM P140916-05414:
	  * If kmalloc fails try with " __GFP_NOFAIL" flag.
	  */
	req->buf = kmalloc(buffer_size, GFP_KERNEL|__GFP_NOWARN);
	if (!req->buf) {
		pr_err("*******[%s]  Kmalloc failed trying with flag __GFP_NOFAIL \n",__func__);
		req->buf = kmalloc(buffer_size, GFP_KERNEL|__GFP_NOFAIL);
		if (!req->buf){
			usb_ep_free_request(ep, req);
			return NULL;
		}
				 ...
				 struct usb_endpoint_descriptor *intr_desc)
{
	...
	for (i = 0; i < DIV_ROUND_UP(MTP_TX_REQ_MAX, 2); i++) {
...
static int mtp_setup(struct usb_composite_dev *cdev)
{
	...
	if (gadget_is_sprd_dwc(gadget)) {
		/* sprd_dwc's max DMA transferable size at once is 64KB-1 */

		/* Even though SPRD supports 64KB, Kiran is low memory device.
		 *
		 * During low-memory conditions, Sometimes kmalloc is failing
		 * for 63K chunks during mtp_init with below error logs
		 *
		 * Normal: 2406*4kB (UEM) 2747*8kB (UEM) 921*16kB (UEM)
		 * 126*32kB (UEM) 3*64kB (M) 0*128kB 0*256kB 0*512kB 0*1024kB
		 * 0*2048kB 0*4096kB = 50560kB
		 *
		 * So, we are reducing the bulk buff size and compensating
		 * the same by increasing the no.of queues
		 */
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
			round_down(0xFFFF, 512);

		/* bulkBufSize & intrBufSize have to smaller
		 * than udc's max transferable size or same
		 * and will be considering memory usage
		 */
		mtpg->bulkBufSize = round_down(MTPG_BULK_BUFFER_SIZE, 512);

		/* MTP use fixed intrBufSize by 22bytes */
		mtpg->intrBufSize = 22;
	} else {
		mtpg->udc_max.txPktSize = mtpg->udc_max.rxPktSize =
