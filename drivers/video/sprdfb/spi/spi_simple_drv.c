...
#ifdef CONFIG_OF
void spi_uninit(struct device *device, u32 spi_id, SPI_INIT_PARM *spi_parm)
#else
void spi_uninit(u32 spi_id, SPI_INIT_PARM *spi_parm)
#endif
{
#ifdef CONFIG_OF
	SPI_Enable(device, false);
#else
	SPI_Enable(spi_id, false);
#endif
}
...
{
	volatile SPI_CTL_REG_T *spi_ctr_ptr = (volatile SPI_CTL_REG_T *)
							(used_spi_reg_base);
