...
#define SHDW_OSD_CTRL				(0x00100)
#define SHDW_OSD_BASE_ADDR		(0x00104)
#define SHDW_OSD_SIZE_XY			(0x00108)
#define SHDW_OSD_PITCH				(0x0010C)
#define SHDW_OSD_DISP_XY			(0x00110)
#define SHDW_OSD_ALPHA				(0x00114)
#define SHDW_OSD_CK					(0x00118)
...
#define dispc_write(value, reg)	\
	do {	\
		...
		pr_debug("dispc_write:reg[0x%x]val[0x%08x][%d %s]\n", \
...
#define dispc_set_bits(bits, reg)	\
	do {	\
		...
		sci_glb_write((g_dispc_base_addr + reg), value, 0xffffffff);	\
		pr_debug("dispc_set_bits:reg[0x%x]val[0x%08x][%d %s]\n", \
				(int)reg, (int)value, __LINE__,  __func__);	\
	} while(0)
...
#define dispc_clear_bits(bits, reg)	\
	do {	\
		uint32_t value = dispc_read(reg) & ~bits;	\
		sci_glb_write((g_dispc_base_addr + reg), value, 0xffffffff);	\
		pr_debug("dispc_clear_bits:reg[0x%x]val[0x%08x][%d %s]\n", \
				(int)reg, (int)value, __LINE__,  __func__);	\
	} while(0)
#else
#define dispc_write(value, reg)	\
