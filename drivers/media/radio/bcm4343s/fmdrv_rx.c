...
*/
int init_start_search(struct fmdrv_ops *fmdev, unsigned short start_freq, unsigned char mode)
{
    ...
    if(mode == FM_TUNER_SEEK_MODE)
    {
        ...
        FM_CHECK_SEND_CMD_STATUS(ret);
#if V4L2_FM_DEBUG
        pr_info("(fmdev) %s(): FM_SEARCH_METHOD set to 0x%x\n", __func__, payload);
#endif
        ...
        FM_CHECK_SEND_CMD_STATUS(ret);
#if V4L2_FM_DEBUG
        pr_info("(fmdev) %s(): FM_REG_PRESET_MAX set to 0x%x\n", __func__, payload);
#endif
        ...
        FM_CHECK_SEND_CMD_STATUS(ret);
#if V4L2_FM_DEBUG
        pr_info("(fmdev) %s(): FM_REG_SCH_CTL0 set to 0x%x\n", __func__, payload);
#endif
    }
    ...
    FM_CHECK_SEND_CMD_STATUS(ret);
#if V4L2_FM_DEBUG
    pr_info("(fmdev) %s(): FM_REG_FM_FREQ set to 0x%x\n", __func__, FM_SET_FREQ(start_freq));
#endif
    ...
    ret = fmc_send_cmd(fmdev, FM_REG_SCH_TUNE, &payload, sizeof(payload),
            REG_WR, &fmdev->maintask_completion, NULL, NULL);
    FM_CHECK_SEND_CMD_STATUS(ret);
    fmdev->rx.curr_search_state = (mode == FM_TUNER_SEEK_MODE)?FM_STATE_SEEKING:FM_STATE_TUNING;
...
int fm_rx_read_curr_rssi_freq(struct fmdrv_ops *fmdev, unsigned char rssi_only)
{
    ...
    FM_CHECK_SEND_CMD_STATUS(ret);
    pr_info("(fmdev) %s(): FM_REG_FM_FREQ : %d\n", __func__, FM_SET_FREQ(tmp_frq));
                            ...
                            unsigned char wrap_around)
{
    ...
    {
        start_freq = (direction_upward)?(fmdev->rx.region.low_bound+ fm_sch_step_size[fmdev->rx.sch_step])    /* darrel issue_06  */
                            :(fmdev->rx.region.high_bound - fm_sch_step_size[fmdev->rx.sch_step]);
        pr_info("(fmdev) %s(): Current frequency is out of bounds. Resetting to freq (%d)\n",
                        __func__, FM_SET_FREQ(start_freq));
    }
    else
    {
        start_freq = (direction_upward)?(tmp_freq + fm_sch_step_size[fmdev->rx.sch_step])    /* darrel issue_06  */
    ...
    pr_info("(fmdrv) %s(): Starting FM seek (%s) from %d..\n",
		__func__, (direction_upward?"SEEKUP":"SEEKDOWN"), FM_SET_FREQ(start_freq));
