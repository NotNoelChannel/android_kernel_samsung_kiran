...
inline void dump_tx_skb_data(struct sk_buff *skb)
{
    ...
    printk(KERN_INFO "<<%shdr:%02x len:%02x opcode:%02x type:%s",
    ...
    while ((skb = skb_dequeue(&fmdev->rx_q)))
    {
        ...
        if (fm_evt_hdr->event_id == HCI_EV_CMD_COMPLETE)
        {
            ...
            /* This is the VSE interrupt handler case */
                                    ...
                                    fmdev->response_completion == NULL) 
            {
                pr_info("(fmdrv) %s : VSE interrupt handler case for 0x%x\n", __func__,
            ...
            int *reponse_len)
{
    ...
    if (fm_evt_hdr->event_id == HCI_EV_CMD_COMPLETE) /* Vendor specific command response */
    {
        ...
        pr_info("(fmdrv) %s(): Reponse status success for 0x%02X: %d\n",
				__func__, fmreg_index, cmd_complete_hdr->status);
                    ...
                    char __user * buf, size_t count)
{
    ...
    spin_unlock_irqrestore(&fmdev->rds_cbuff_lock, flags);
#if V4L2_RDS_DEBUG
    pr_info("(fm_rds) %s(): Done copying %d\n", __func__, ret);
#endif
    return ret;
}
...
#if V4L2_FM_DEBUG
    pr_debug("(fmdrv) %s(): FM_REG_RDS_SYS write done\n", __func__);
#endif
...
static long fm_st_receive(void *arg, struct sk_buff *skb)
{
    ...
    if (skb->cb[0] != FM_PKT_LOGICAL_CHAN_NUMBER) {
        ...
        for (len = 0; ((skb) && (len < skb->len)); len++)
            pr_debug(">> 0x%02x ", skb->data[len]);
#endif

    memcpy(skb_push(skb, 1), &pkt_type, 1);
    skb_queue_tail(&fmdev->rx_q, skb);
    pr_info("(fmdrv) %s(): fm_st_receive: schedule recv tasklet\n",
							__func__);
