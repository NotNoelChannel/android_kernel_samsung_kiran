...
LOCAL int sprd_v4l2_setflash(uint32_t flash_mode)
{
	...
	case FLASH_CLOSE:
	 ...
	 /*
	  * if assistive light is off then only close flash by camera request
	  * if assistive light is on  then dont close flash by camera request
	  * if end user make flash on by notification bar(assistive light)
	  * then end user must off flash by notification bar(assistive light)
	  * higher priority given to notifcation bar (assistive light).
	  */
	...
	while (1) {
		if (0 == down_interruptible(&dev->flash_thread_sem)) {
			if (dev->is_flash_thread_stop) {
				/*
				case 1: when assistive light is on and camera exits, assistive light should not be closed forcefully
				case 2: when camera flash is on, and if camera exits abruptly then flash should be closed forcefully
				*/
				if((dev->dcam_cxt.flash_status != FLASH_STATUS_MAX) && !(flash_torch_status))
			...
			struct v4l2_streamparm *streamparm)
{
	...
	case CAPTURE_SET_FLASH:
		...
		if(dev->dcam_cxt.flash_status==FLASH_OPEN ||dev->dcam_cxt.flash_status==FLASH_CLOSE_AFTER_OPEN
			|| dev->dcam_cxt.flash_status==FLASH_CLOSE || dev->dcam_cxt.flash_status==FLASH_TORCH || dev->dcam_cxt.flash_status==FLASH_HIGH_LIGHT)
		{
			sprd_v4l2_setflash(dev->dcam_cxt.flash_status);
		}
		/*if(dev->dcam_cxt.flash_status==FLASH_OPEN)
		{
			sprd_v4l2_setflash(FLASH_OPEN);
		}
		if(dev->dcam_cxt.flash_status==FLASH_CLOSE_AFTER_OPEN)
		{
			sprd_v4l2_setflash(FLASH_CLOSE_AFTER_OPEN);
		}*/
		mutex_unlock(&dev->dcam_mutex);
