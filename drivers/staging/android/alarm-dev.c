...
static struct wakeup_source alarm_rtc_lock;
...
static void devalarm_start(struct devalarm *alrm, ktime_t exp)
{
	if (is_wakeup(alrm->type)) {
		__pm_stay_awake(&alarm_rtc_lock);
		alarm_start(&alrm->u.alrm, exp);
		__pm_relax(&alarm_rtc_lock);
	} else
...
static void devalarm_triggered(struct devalarm *alarm)
{
	...
	spin_lock_irqsave(&alarm_slock, flags);
	if (alarm_enabled & alarm_type_mask) {
		__pm_wakeup_event(&alarm_wake_lock, 3000); /* 3secs */
		alarm_dbg(INFO, "%s: wakeup_event keep 3s\n", __func__);
...
static int __init alarm_dev_init(void)
{
	...
	wakeup_source_init(&alarm_rtc_lock, "alarm-rtc-lock");
	return 0;
}
...
static void  __exit alarm_dev_exit(void)
{
	...
	wakeup_source_trash(&alarm_rtc_lock);
}
