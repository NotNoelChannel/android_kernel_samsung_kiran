...
{
#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_PARTIALSUSPEND_SLP)
	return 0;
#else
	int err=-1;
	sprd_iommu_gsp_enable(dev);
...
int sprd_iommu_gsp_restore(struct sprd_iommu_dev *dev)
{
	...
	int err = -1;
#ifdef GSP_IOMMU_WORKAROUND1
	err = sprd_iommu_gsp_enable_withworkaround(dev);
#else
	err = sprd_iommu_gsp_enable(dev);
#endif
	err = sprd_iommu_restore(dev);
#endif
	err = sprd_iommu_gsp_disable(dev);
	return err;
}
