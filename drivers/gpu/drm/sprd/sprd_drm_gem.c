...
void sprd_drm_gem_destroy(struct sprd_drm_gem_obj *sprd_gem_obj)
{
	...
	DRM_DEBUG_KMS("%s:obj[0x%x]addr[0x%x]\n", __func__,
				...
				struct drm_file *file_priv)
{
	...
	DRM_DEBUG_KMS("%s:hdl[%d]sz[%d]f[0x%x]obj[0x%x]addr[0x%x]\n",
				...
				struct drm_file *file_priv)
{
	...
	DRM_INFO("%s:hdl[%d]cnt[%d]sz[%d %d %d]f[0x%x]obj[0x%x]addr[0x%x]\n",
					...
					struct drm_file *file_priv)
{
	...
	if (sprd_gem_obj->flags & SPRD_BO_NONCONTIG) {
		...
		if (sprd_unmap_iommu(ion_handle, domain_num))
			DRM_ERROR("failed to unmap iommu\n");
	}
...
void *sprd_drm_gem_get_obj_addr(unsigned int name, unsigned int index)
{
	...
	DRM_DEBUG_KMS("%s:name[%d]obj[0x%x]idx[%d]addr[0x%x]\n",
                                      ...
                                      struct drm_file *file_priv)
{
	...
	if (obj == NULL) {
		DRM_ERROR("cache op Invalid GEM handle %x\n", args->gem_handle);
		result = -EBADF;
		goto out;
	}
	...
	if (IS_NONCACHABLE_BUFFER(sprd_gem_obj->flags))
		return 0;

	switch (cache_op) {
		case SPRD_DRM_CACHE_INV:
			dma_sync_sg_for_cpu(NULL, sprd_gem_obj->buffer->sgt->sgl,
					sprd_gem_obj->buffer->sgt->nents, DMA_FROM_DEVICE);
			break;
		case SPRD_DRM_CACHE_CLN:
		case SPRD_DRM_CACHE_FSH:
			dma_sync_sg_for_device(NULL, sprd_gem_obj->buffer->sgt->sgl,
					sprd_gem_obj->buffer->sgt->nents, DMA_BIDIRECTIONAL);
			break;
		default:
			DRM_ERROR("invalid cache op.\n");
			result = -EINVAL;
			goto out;
	}
