...
/* gsp debugging - to dump register contents */
#ifdef CONFIG_GSP_IPPDRV_DEBUG
#define SPRD_DRM_GSP_REGS_SIZE 140
#define SPRD_DRM_GSP_REGS_BASE_ADDR (0x20a00000)
#endif
...
static uint8_t _init_pool(void *buffer_ptr, uint32_t buffer_size, GSC_MEM_POOL * pool_ptr)
{
	if (!buffer_ptr || !buffer_size || !pool_ptr) {
		DRM_ERROR("input param is null\n");
		return false;
	}

	if (buffer_size < MIN_POOL_SIZE) {
		DRM_ERROR("invalid buffer_size[%d]\n", buffer_size);
		return false;
	}
	...
	DRM_DEBUG("%s:begin_addr[0x%08x]total_size[%d]used_size[%d]\n",
		__func__, pool_ptr->begin_addr,
...
static void *_allocate(uint32_t size, uint32_t align_shift, GSC_MEM_POOL * pool_ptr)
{
	...
	if (temp_addr + size > begin_addr + pool_ptr->total_size) {
		DRM_ERROR("temp_addr[0x%08x]size[%d]begin_addr[0x%08x]"
...
static int64_t div64_s64_s64(int64_t dividend, int64_t divisor)
{
	...
	int64_t divisor_tmp = divisor, ret = 0;
...
static int16_t cal_y_mode_l_coef(int16_t coef_lenght, int16_t * coef_data_ptr, int16_t N, int16_t M,
				GSC_MEM_POOL * pool_ptr)
{
	...
	if (!filter || !tmp_filter || !normal_filter)
		return 1;
...
/* cal Y model */
static int16_t cal_y_scaling_coef(int16_t tap, int16_t D, int16_t I, int16_t * y_coef_data_ptr,
...
static int16_t cal_uv_scaling_coef(int16_t tap, int16_t D, int16_t I, int16_t * uv_coef_data_ptr,
				int16_t dir, GSC_MEM_POOL * pool_ptr)
{
...
static void get_filter(int16_t * coef_data_ptr, int16_t * out_filter, int16_t iI_hor, int16_t coef_len,
			int16_t * filter_len)
{
	int16_t i, pos_start;
...
static void write_scalar_coef(int16_t * dst_coef_ptr, int16_t * coef_ptr, int16_t dst_pitch,
				int16_t src_pitch)
{
	int i, j;
...
static int32_t cache_coef_init(void)
{
	...
	if (s_cache_coef_init_flag == 0) {
		Coef_Entry_Array = (Coef_Entry *) vmalloc(
				...
				void *temp_buf_ptr, uint32_t temp_buf_size)
{
	...
	if (!_init_pool(temp_buf_ptr, temp_buf_size, &pool)) {
		DRM_ERROR("failed to _init_pool\n");
		return false;
	}
	...
	if (!cong_com_hor || !cong_com_ver || !coeff_array) {
		DRM_ERROR("_allocate failed:0x%08x,0x%08x,0x%08x\n",
				(int)cong_com_hor, (int)cong_com_ver, (int)coeff_array);
		return false;
	}
	...
	if (!temp_filter_ptr || !filter_ptr) {
		DRM_ERROR("_allocate failed\n");
		return false;
	}
	...
	if (s_cache_coef_init_flag == 1) {
		...
		if (pEntry->in_w == 0)
			DRM_DEBUG("%s:GSP_CACHE_COEF:add\n", __func__);
		else
			DRM_DEBUG("%s:GSP_CACHE_COEF:swap\n", __func__);
		...
		u32 fmt, struct drm_sprd_sz *sz)
{
	...
	case DRM_FORMAT_XRGB8888:
		...
		if (*base[0]) {
			if (size[0] < ofs[0]) {
				DRM_INFO("%s:size mismatch issues\n", __func__);
				/* FIXME:!! size is always taken as destination size,
					even for source buffer. Need to fix this */
#if 0
				 goto err_info;
#endif
			}
		...
		struct gsp_context *ctx)
{
	...
	if (gsp_cfg->layer0_info.scaling_en == 1) {
		...
		if (ctx->coef_force_calc ||coef_in_w_last != coef_in_w
			...
			if (!tmp_buf) {
				DRM_ERROR("no mem to alloc coeff buffer\n");
			...
			if (!(gsp_gen_block_ccaler_coef(coef_in_w, coef_in_h, coef_out_w,
					...
					GSP_COEFF_POOL_SIZE))) {
				kfree(tmp_buf);
				DRM_ERROR("failed to gsp_gen_block_ccaler_coef\n");
...
#ifdef CONFIG_GSP_IPPDRV_DEBUG
static void gsp_ippdrv_dump_regs()
{
	void __iomem *base;
	unsigned long *ptr;
	size_t index, size = SPRD_DRM_GSP_REGS_SIZE;

	base = ioremap(SPRD_DRM_GSP_REGS_BASE_ADDR, size);
	if (!base)
		return;
	ptr = (unsigned long *)base;

	DRM_INFO("%s : ++\n", __func__);
	for (index = 0; index <= SPRD_DRM_GSP_REGS_SIZE; index+=4 , ++ptr)
		DRM_INFO("%s: Offset[0x%x] Value[0x%x]\n", __func__, index, *ptr);
	DRM_INFO("%s : --\n", __func__);
...
/* FIXME:!! Eventhough gsp hardware gives proper output, it sets the error
	flag in the SPRD_GSP_CFG register and error code = 3 (scale not set
	in x/y co-ordinate. This is not observed when we use sprd legacy
	gsp driver */
#if 0
		cfg = gsp_read(SPRD_GSP_CFG);
		...
		if (SPRD_GSP_CFG_ERR_FLAG_GET(cfg)) {
			...
			DRM_ERROR("configuration error[%u]\n", cfg);

			return SPRD_GSP_CFG_ERR_CODE_GET(cfg);
		}
#endif
#ifdef CONFIG_GSP_IPPDRV_DEBUG
		gsp_ippdrv_dump_regs();
#endif
		cfg = gsp_read(SPRD_GSP_CFG);
...
#ifdef CONFIG_OF
	emc_clk_parent = of_clk_get_by_name(ctx->gsp_of_dev->of_node,
			GSP_EMC_CLOCK_PARENT_NAME);
#else
	emc_clk_parent = clk_get(NULL, GSP_EMC_CLOCK_PARENT_NAME);
#endif
	if (IS_ERR(emc_clk_parent)) {
		DRM_ERROR("get emc clk_parent failed\n");
		return -1;
	}

#ifdef CONFIG_OF
	ctx->emc_clk = of_clk_get_by_name(ctx->gsp_of_dev->of_node,
			GSP_EMC_CLOCK_NAME);
#else
	ctx->emc_clk = clk_get(NULL, GSP_EMC_CLOCK_NAME);
#endif
	if (IS_ERR(ctx->emc_clk)) {
		DRM_ERROR("get emc clk failed\n");
		return -1;
	}

	ret = clk_set_parent(ctx->emc_clk, emc_clk_parent);
	if (ret) {
		DRM_ERROR("set emc clk parent failed\n");
		return -1;
	}

#ifdef CONFIG_OF
	gsp_clk_parent = of_clk_get_by_name(ctx->gsp_of_dev->of_node,
			GSP_CLOCK_PARENT3);
#else
	gsp_clk_parent = clk_get(NULL, GSP_CLOCK_PARENT3);
#endif
	if (IS_ERR(gsp_clk_parent)) {
		DRM_ERROR("get clk_parent failed\n");
		return -1;
	}

#ifdef CONFIG_OF
	ctx->gsp_clk = of_clk_get_by_name(ctx->gsp_of_dev->of_node,
			GSP_CLOCK_NAME);
#else
	ctx->gsp_clk = clk_get(NULL, GSP_CLOCK_NAME);
#endif
	if (IS_ERR(ctx->gsp_clk)) {
		DRM_ERROR("get clk failed\n");
		return -1;
	}

	ret = clk_set_parent(ctx->gsp_clk, gsp_clk_parent);
	if (ret) {
		DRM_ERROR("gsp set clk parent failed\n");
		return -1;
	}
...
static int gsp_probe(struct platform_device *pdev)
{
	...
	ret = of_address_to_resource(ctx->gsp_of_dev->of_node, 0, &r);
	if(ret) {
		DRM_ERROR("can't get register base addr:ret[%d]\n", ret);
		return ret;
	}

	ctx->suspended = true; /* gsp device in off state */

	ctx->regs = (void __iomem *)r.start;
	...
	ret = of_property_read_u32(ctx->gsp_of_dev->of_node, "gsp_mmu_ctrl_base",
			&gsp_mmu_ctrl_addr);
	DRM_DEBUG("%s:gsp_mmu_ctrl_addr[0x%x]\n", __func__, gsp_mmu_ctrl_addr);
	if(ret) {
...
#endif
#endif
	DRM_DEBUG("%s:irq[%d]gsp_base_addr[0x%x]\n", __func__, ctx->irq,
			(int)ctx->regs);
#else
