...
#ifdef CONFIG_BATTERY_DETECT_SPRD27X3
static int is_no_bat(void)
{
	int adc_value = 0;

retry_adc:
	adc_value = sci_adc_get_value(ADC_CHANNEL_0, false);
	if (adc_value<0) {
		pr_info("%s:ADC read error\n", __func__);
		udelay(10);
		goto retry_adc;
	} else {
		pr_info("%s:adc_value=%x\n", __func__, adc_value);
		if (adc_value == 0xfff)
			return 0;
		else
			return 1;
