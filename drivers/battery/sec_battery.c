 ...
 */
#if defined(CONFIG_BAT_DET_IRQ_FROM_SC2723)
				...
				struct sec_battery_info *battery, int seconds)
{
	...
	ret = alarm_start(&battery->polling_alarm,
		    ktime_add(battery->last_poll_time, ktime_set(seconds, 0)));

	dev_info(battery->dev, "%s:ret(%d)\n", __func__, ret);
#endif
}
...
#endif
{
	...
	dev_info(battery->dev,
		"%s:polling_in_sleep(%d), Sleep:%s\n",
		__func__, battery->polling_in_sleep, battery->polling_in_sleep ? "Yes" : "No");
#if !defined(ANDROID_ALARM_ACTIVATED)
	return ALARMTIMER_NORESTART;
#endif
}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		power_supply_changed(&battery->psy_bat);
		break;
#if defined(CONFIG_BAT_DET_IRQ_FROM_SC2723) || defined(CONFIG_BAT_DET_IRQ_FROM_SM5701)
...
static irqreturn_t sec_batt_detect_irq(int irq, void *dev_id)
{
	...
	ps_val.intval = gpio_get_value(irq);
	pr_info("%s: Battery Present = %d\n", __func__,ps_val.intval);

	irq_set_irq_type(irq, ps_val.intval  ? IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH);
...
static int sec_battery_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(battery->irq, sec_batt_detect_irq, IRQF_SHARED | IRQF_TRIGGER_LOW, "batt_detect_irq", battery);
#endif
