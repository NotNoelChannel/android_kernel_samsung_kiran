				...
				int depth, void *data)
{
	...
	prop = of_get_flat_dt_prop(node, "sprd,ion-heap-mem", &len);
	...
	name = of_get_flat_dt_prop(node, "reg-names", NULL);
...
void cma_assign_device_from_dt(struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node,
				"linux,contiguous-region", &value);
	if (ret)
		return;

	if (of_property_read_string(dev->of_node, "reg-names", &name))
		return;
	...
	pr_info("Assigned CMA region to %s device\n", dev_name(dev));
}
