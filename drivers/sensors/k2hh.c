...
static int k2hh_read_accel_xyz(struct k2hh_p *data, struct k2hh_v *acc)
{
	...
	if (data->orientation[0]) {
		acc->x *= data->orientation[0];
		acc->y *= data->orientation[4];
	} else {
		temp = acc->x*data->orientation[1];
		acc->x = acc->y*data->orientation[3];
		acc->y = temp;
	}
	acc->z *= data->orientation[8];
#else
...
static void k2hh_work_func(struct work_struct *work)
{
	...
	if (ret < 0) {
		pr_info("[SENSOR] %s, data read failed\n", __func__);
		goto exit;
	}
...
exit:
	if (((int64_t)atomic_read(&data->delay) * (int64_t)data->time_count)
		>= ((int64_t)ACCEL_LOG_TIME * MSEC_PER_SEC)) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&data->delay)*1000000);
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	atomic_set(&data->delay, (int)delay/1000000);
	k2hh_delay_change(data);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if((acc.x == 0) && (acc.y == 0) && (acc.z == 0)) {
		pr_info("[SENSOR]: %s - x = y = z = 0, enable = %u, abnormal status!!!\n", __func__, atomic_read(&data->enable));
	}
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((enable == ON) && (data->recog_flag == OFF)) {
		...
		if (factory_mode == ON) {
			...
			msleep(20);
			k2hh_read_accel_xyz(data, (struct k2hh_v *)temp);
...
static int k2hh_setup_pin(struct k2hh_p *data)
{
	...
	pr_info("###### [SENSOR]: %s setup ######\n", __func__);

	ret = gpio_request(data->acc_int1, "k2hh");
	if (ret < 0) {
	...
	ret = request_threaded_irq(data->irq1, NULL, k2hh_irq_thread,
		IRQF_TRIGGER_RISING | IRQF_ONESHOT | IRQF_NO_SUSPEND, "k2hh", data);
	if (ret < 0) {
		...
		const struct i2c_device_id *id)
{
...
#else
	struct k3dh_platform_data *platform_data = client->dev.platform_data;
#endif
	...
	if (client->dev.of_node){
		data->acc_int1 = of_get_gpio(np, 0);
		printk("[K2HH] gpio id=%d", data->acc_int1);
	}

	ret = of_property_read_u32_array(client->dev.of_node, "orientation", data->orientation, 9);
	if(ret) {
		pr_err("%s: Unable to find orientation key, ret=%d", __func__, ret);
		goto exit_kzalloc;
	}
#else
