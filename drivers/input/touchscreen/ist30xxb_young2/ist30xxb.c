...
static int ist30xx_open(struct input_dev *input)
{
...
static void ist30xx_close(struct input_dev *input)
{
	struct device *dev = input->dev.parent;
	struct ist30xx_data *data = dev_get_drvdata(dev);
...
#endif
	dev_set_drvdata(input_dev, data);
...
#if defined(CONFIG_MACH_VIVALTO) || defined(CONFIG_MACH_KIRAN_WVGA)
	if (get_hw_rev() > 0x00)
...
#ifdef CONFIG_PM_SLEEP
	data->input_dev->open = ist30xx_open;
	data->input_dev->close = ist30xx_close;
#endif
