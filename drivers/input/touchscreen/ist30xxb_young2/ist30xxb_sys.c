...
static void ts_power_enable(int en)
{
	...
	if (touch_regulator_3v3 ==  NULL) {
		...
		if (IS_ERR(touch_regulator_3v3)) {
			...
			tsp_err("get touch_regulator_1v8 regulator error\n");
			return;
		}
	}
	...
	if(en) {
		...
		if (ret)
			tsp_err(KERN_ERR "%s: touch_regulator_1v8 enable failed (%d)\n",__func__, ret);
	
		regulator_set_voltage(touch_regulator_3v3, 3000000, 3000000);
		ret = regulator_enable(touch_regulator_3v3);
		if (ret)
			tsp_err(KERN_ERR "%s: touch_regulator_3v0 enable failed (%d)\n",__func__, ret);
	} else {
		ret = regulator_disable(touch_regulator_3v3);
		if (ret)
			tsp_err(KERN_ERR "%s: touch_regulator_3v0 disable failed (%d)\n",__func__, ret);
	}
}
