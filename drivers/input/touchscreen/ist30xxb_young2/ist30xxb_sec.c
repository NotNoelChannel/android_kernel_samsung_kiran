...
#define IST_RX_NUM	10
#define IST_TX_NUM	15
...
u32 ist30xxb_get_fw_ver(struct ist30xx_data *data)
{
	...
	ret = ist30xx_read_cmd(data->client, (addr-4)+0x108,
							&data->threshold);
	if (ret) goto get_fw_ver_fail;

	ret = ist30xx_read_cmd(data->client, (addr-4)+0x134,
							&data->tkey_threshold);
	if (ret) goto get_fw_ver_fail;
...
void get_tsp_status(void *dev_data)
{
	int ret = 0;
	char buf[16] = { 0 };
...
/* FIXME:!! ist30xx_read_cmd is succesful, tsp_type is returned as UNKNOWN */
#if 0
	if (tsp_check == true)
...
struct tsp_cmd tsp_cmds[] = {
	...
	{ TSP_CMD("tsp_connect_test",   get_tsp_status),   },
...
#if !defined (CONFIG_MACH_YOUNG2) && defined(CONFIG_MACH_KIRAN)
