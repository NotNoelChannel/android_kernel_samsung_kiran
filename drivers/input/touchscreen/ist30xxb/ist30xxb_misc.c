...
int ist30xx_parse_touch_node(u8 flag, struct TSP_NODE_BUF *node)
{
	...
	for (i = 0,k = 0; i < node->len; i++) {
		if (flag & (NODE_FLAG_RAW | NODE_FLAG_BASE)) {
			*raw++ = ist30xx_frame_rawbuf[i] & 0xFFF;
			*base++ = (ist30xx_frame_rawbuf[i] >> 16) & 0xFFF;
		}
		if (flag & NODE_FLAG_FILTER){
			*filter++ = ist30xx_frame_fltbuf[k++] & 0xFFF;
				 ...
				 const char *buf, size_t size)
{
	...
	ts_data->ist30xx_report_rate = rate;
				     ...
				     const char *buf, size_t size)
{
	...
	ts_data->ist30xx_idle_rate = rate;
	schedule_delayed_work(&ts_data->work_reset_check, 0);
