...
#if defined(TOUCH_FB_NOTIFIER)
static void disable_sensor(struct ist30xx_data *data)
{
	mutex_lock(&ist30xx_mutex);
	ist30xx_disable_irq(data);
	ist30xx_internal_suspend(data);
...
static void enable_sensor(struct ist30xx_data *data)
{
	mutex_lock(&ist30xx_mutex);
	ist30xx_internal_resume(data);
	ist30xx_start(data);
...
static int ist_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *fb_evdata)
{
	...
	if ((event != FB_EVENT_BLANK) || !self || !evdata)
		goto exit;

	data = container_of(self, struct ist30xx_data, fb_notif);
	if (!data)
		goto exit;
	...
	switch (blank)
		...
		case FB_BLANK_UNBLANK:
			enable_sensor(data);
			printk("[ TSP ] %s(): Sensor enabled !!\n", __func__);
			break;
		case FB_BLANK_POWERDOWN:
			disable_sensor(data);
			printk("[ TSP ] %s(): Sensor disabled !! \n", __func__);
			break;
		default:
			printk("[ TSP ] %s(): invalid blank event !! \n", __func__);
			break;
	}
exit:
	return retval;
}
static int ist_register_fb(struct ist30xx_data *data)
{
	memset(&data->fb_notif, 0, sizeof(data->fb_notif));
	data->fb_notif.notifier_call = ist_fb_notifier_callback;
	return fb_register_client(&data->fb_notif);
}
static void ist_unregister_fb(struct ist30xx_data *data)
{
	fb_unregister_client(&data->fb_notif);
}
#endif	/* TOUCH_FB_NOTIFIER */
