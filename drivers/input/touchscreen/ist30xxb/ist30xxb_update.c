...
#include "ist30xxb_FW_Young2dtv_Ver0001.h"
#elif  defined(CONFIG_MACH_KIRAN)
#include "IST30xxB_FW_Vivalto.h"
#endif
...
//	ist30xx_reset();
	...
	if ((fw->param_ver > 0) && (fw->param_ver < 0xFFFFFFFF)) {
		...
		if (fw->param_ver <= 0x03){
			fw->buf = (u8 *)ist30xxb_fw_00;
			fw->buf_size = sizeof(ist30xxb_fw_00);
		if (fw->param_ver < 0x03){
			printk("[TSP] %s vivalto TSP IC is old, have to update 0x03! line# %d\n ",__func__,__LINE__);
			goto fw_check_end;
		}
		}
		if(fw->param_ver < 0x10){
			printk("[TSP] %s vivalto TSP IC is old, don't update FW! line# %d\n ",__func__,__LINE__);
			return 0;      //done to prevent FW update
		}
#elif defined(CONFIG_MACH_CORSICA_VE)
		if(get_hw_rev()<0x07){
			if (fw->param_ver <= 0x09){
				fw->buf = (u8 *)ist30xxb_fw_00;
				fw->buf_size = sizeof(ist30xxb_fw_00);
				if (fw->param_ver < 0x09){
					printk("[TSP] %s corsicave TSP IC is old, have to update 0x09! line# %d\n ",__func__,__LINE__);
					goto fw_check_end;
				}
			}
			if(fw->param_ver < 0x30){
				printk("[TSP] %s corsicave TSP IC is old, don't update FW! line# %d\n ",__func__,__LINE__);
				return 0;      //done to prevent FW update
			}
		}
#endif
...
#if IST30XX_SUPPORT_SUB_VER
	tsp_info("BIN: %x, Binary ver core: %x, param: %x, sub: %x\n",
		 data->chip_id, ist30xx_fw_ver, ist30xx_param_ver, ist30xx_sub_ver);
#else
	tsp_info("BIN: %x, Binary ver core: %x, param: %x\n",
...
#else
	count = sprintf(buf, "ID: %x, f/w core: %x, param: %x\n",
