				...
				struct ff_effect *effect)
{
	...
	if (haptic->level != haptic->state) {
           printk(KERN_WARNING "MyChanges: in function ff_haptic_play level %d state %d\n", haptic->level, haptic->state);
           haptic->state = haptic->level;
