...
#ifdef CONFIG_BCM_BT_LPM
static int bcm_bt_lpm_open(void);
static void bcm_bt_lpm_close(void);
#endif
...
static enum hrtimer_restart bt_lpm_sleep(struct hrtimer *htimer)
{
	struct bt_lpm_timer *timer = container_of(htimer, struct bt_lpm_timer,
				htimer);
	struct bcm_bt_lpm_entry_struct *pdata = container_of(timer,
				struct bcm_bt_lpm_entry_struct, timer);
	bcm_bt_lpm_assert_bt_wake();
	//pm_wakeup_event(&pdata->pdev->dev, HZ / 2);
	pm_relax(&pdata->pdev->dev);
	return HRTIMER_NORESTART;
}

static void bt_lpm_wake(struct bcm_bt_lpm_entry_struct *data)
{
	struct hrtimer *htimer = &data->timer.htimer;

	hrtimer_try_to_cancel(htimer);
	pm_stay_awake(&data->pdev->dev);
	bcm_bt_lpm_deassert_bt_wake();
	hrtimer_start(htimer, data->timer.sleep_delay, HRTIMER_MODE_REL);
}

static int bt_lpm_init(struct bcm_bt_lpm_entry_struct *data)
{
	struct hrtimer *timer = &data->timer.htimer;

	hrtimer_init(timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	timer->function = bt_lpm_sleep;
	data->timer.sleep_delay = ktime_set(BT_PM_WAKE_DELAY, 0);
...
//	wake_unlock(&priv_g->bt_wakelock);
	...
	pr_debug("moonys -----bcm_bt_lpm_assert_bt_wake:%s\n",
		gpio_get_value(priv_g->pdata->bt_wake_gpio) ? "High [POWER ON]" :
		"Low [POWER_OFF]");
	return 0;
}
...
int bcm_bt_lpm_deassert_bt_wake(void)
{
	...
	pr_debug("moonys -----bcm_bt_lpm_deassert_bt_wake: value %d %s\n",priv_g->pdata->bt_wake_gpio,
		gpio_get_value(priv_g->pdata->bt_wake_gpio) ? "High [POWER ON]" :
		"Low [POWER_OFF]");
        //wake_unlock(&priv_g->bt_wakelock);
	return 0;
...
static int bcm_bt_lpm_probe(struct platform_device *pdev)
{
	...
	bcm_bt_lpm_open();
	bt_lpm_init(priv_g);
	device_init_wakeup(&priv_g->pdev->dev, true);
#endif
...
static int bcm_bt_lpm_suspend(struct platform_device *pdev)
{
	pr_info(" suspending\n");
	if (bt_status == 0)
		bcm_bt_lpm_assert_bt_wake();
                                ...
                                unsigned long event, void *data)
{
        switch (event) {
        case HCI_DEV_REG:
			pr_debug("HCI_DEV_REG event\n");
                break;
        case HCI_DEV_UNREG:
			pr_debug("HCI_DEV_UNREG event\n");
                break;
        case HCI_DEV_WRITE:
			bt_lpm_wake(priv_g);
			pr_debug("HCI_DEV_WRITE event\n");
            break;
        }
...
#ifdef CONFIG_BCM_BT_LPM
	pr_debug(" bcm : hci event register\n");
...
static void __exit bcm_bt_lpm_exit(void)
{
	...
	printk("%s BLUETOOTH:Exiting.\n", __func__);
#ifdef CONFIG_BCM_BT_LPM
	pr_debug("bcm unregister hci event handler\n");
	bcm_bt_lpm_close();
