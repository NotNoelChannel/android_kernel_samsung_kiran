...
static int debugfs_ana_addr_get(void *data, u64 * val)
{
	if (ana_addr < PAGE_SIZE) {
		pr_info("%s: try to get vr_addr :0x%0x\n",
			__func__, SCI_ADDR(ANA_REGS_GLB_BASE, ana_addr));
		*val = ANA_REG_GET(SCI_ADDR(ANA_REGS_GLB_BASE, ana_addr));
	} else {
...
{
	if (ana_addr < PAGE_SIZE) {
		pr_info("%s: try to set vr_addr :0x%0x, by %lu\n",
			__func__, SCI_ADDR(ANA_REGS_GLB_BASE, ana_addr), val);
		ANA_REG_SET(SCI_ADDR(ANA_REGS_GLB_BASE, ana_addr), val, -1);
	} else {
		void *addr = ioremap(ana_addr, PAGE_SIZE);
