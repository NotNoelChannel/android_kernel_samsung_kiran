...
#define USE_SUSPEND_SYSTEM_TIME
#undef USE_SUSPEND_PERSISTCLOCK_TIME
#undef USE_SUSPEND_RTC_TIME
...
#endif
	char wakeup_by_interrupt[WAKEUP_BY_STRING];
};
...
static struct sleep_history_data sleep_history_datas;
...
void wakeupby_integer_to_char(char *char_str, int digit)
{
	int backup_digit;
	int count=0;
	...
	while(true){
		if(digit >= 10){
			if(count > 0)
				char_str[--count] = digit%10 + 48;
			else
				break;
			digit = digit/10;
		}else{
			char_str[--count] = digit + 48;
			break;
		}
	}
}

int wakeupby_string_legth(char *str3)
{
	int i=0;
	while(true){
		if(str3[i]=='\0')
...
void wakeupby_string_add(char *str1, char *str2)
{
	int i=0, k=0;
	if(NULL!=str1){
		while(true){
	...
	for(k=0; k < wakeupby_string_legth(str2); k++){
		str1[i+k] =str2[k];
	}
	str1[i+k]='\0';
...
static int print_wakeup_by_something(
	struct seq_file *s, union wakeup_history *wakeup)
{
	seq_printf(s, "%s", wakeup->wakeup_by_interrupt);
	return 0;
}
...
static int copy_sleep_history(struct sleep_history *buf, int count)
{
	...
	ring_buf = (struct sleep_history *)(sleep_history_datas.sleep_history.buf);
	head = sleep_history_datas.sleep_history.head;
	tail = sleep_history_datas.sleep_history.tail;
...
{
	int i, index, wakeup_count = 0, err = -ENODEV;
	int k=0;
	...
	wakeup_interrupt = (char *)vmalloc(sizeof(char)*WAKEUP_BY_STRING);
	...
	for (i = 0, index = 1; i < SLEEP_HISTORY_RINGBUFFER_SIZE-1; i ++) {
		...
		if ((sleep_history + i)->type == SLEEP_HISTORY_AUTOSLEEP_ENTRY) {
			...
			if ((sleep_history + i)->type == SLEEP_HISTORY_AUTOSLEEP_ENTRY) {
				if ((sleep_history + i)->failed_step > 0){
					...
					seq_printf(s, "%3d %9s           ", index++, type_text[3]);
					is_wakeup = true;
				 }
				...
				print_sleep_history_battery(s, &batt_entry, &batt_exit);
				if(true == is_wakeup){
					print_wakeup_by_something(s,  &((sleep_history + i)->ws));
				}
		...
		if ((sleep_history + i)->type == SLEEP_HISTORY_SUSPEND_SYSTEM_ENTRY ||
			...
			wakeup_count = 0;
			memset(wakeup, 0, sizeof(wakeup));
			memset(wakeup_interrupt, 0, sizeof(wakeup_interrupt));
			do {
				if (!(++i < SLEEP_HISTORY_RINGBUFFER_SIZE-1))
					goto end_ring_buf;
				if ((sleep_history + i)->type == SLEEP_HISTORY_WAKEUP_IRQ){
					for(k=0; k < WAKEUP_BY_STRING; k++)
						wakeup_interrupt[k] = ((sleep_history + i)->ws).wakeup_by_interrupt[k];
					wakeup[wakeup_count++] = &((sleep_history + i)->ws);
				}else
					break;
			} while (wakeup_count < WS_ARRAY_MAX);
			...
			print_sleep_history_battery(s, &batt_entry, &batt_exit);
			if(true == is_wakeup){
				seq_printf(s, "%s", wakeup_interrupt);
			}
end_ring_buf:
			seq_printf(s, "\n");
		}
...
int sleep_history_marker(int type, struct timespec *ts, void *wakeup)
{
	...
	ring_buf_ptr = (struct sleep_history *)(sleep_history_datas.sleep_history.buf);
	head_ptr = &sleep_history_datas.sleep_history.head;
	tail_ptr =	&sleep_history_datas.sleep_history.tail;
	...
	case SLEEP_HISTORY_WAKEUP_IRQ:
		...
		if (wakeup) {
			int i=0;
			wakeup_ptr = &((ring_buf_ptr + *head_ptr)->ws);
			memset (wakeup_ptr->wakeup_by_interrupt, 0, WAKEUP_BY_STRING);
			for(i=0; i < WAKEUP_BY_STRING; i++)
				wakeup_ptr->wakeup_by_interrupt[i] = ((char *)wakeup)[i];
		}
		break;
...
static int sleep_history_syscore_init(void)
{
	...
	sleep_history_datas.sleep_history.head = 0;
	sleep_history_datas.sleep_history.tail = 0;
	sleep_history_datas.sleep_history.buf = (char *)kmalloc(sizeof(struct sleep_history) *
...
#endif
	kfree(sleep_history_datas.sleep_history.buf);
}
