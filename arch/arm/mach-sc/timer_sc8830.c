...
cycle_t __gptimer_clocksource_read(struct clocksource *cs)
{
	...
	unsigned long v, start_v, end_v, old_v;

	spin_lock_irqsave(&clocksource_read_lock, flags);
	v = start_v = ~readl_relaxed(TIMER_CNT_RD(e_cpu, SOURCE_TIMER));
	old_v = last_v;

	/* W/A for abnormal cnt case*/
	dt = (long)(v-last_v);
	while ((dt < 0) && (retry_cnt < MAX_RECHECK_CNT)) {
		v = ~readl_relaxed(TIMER_CNT_RD(e_cpu, SOURCE_TIMER));
		dt = (long)(v-last_v);
		retry_cnt++;
		udelay(20);
		pr_debug("%s: v = 0x%08lx, last_v = 0x%08lx, dt = %ld, cnt:%d\n",
			 __func__, v, last_v, dt, retry_cnt);
	}
	end_v = v;

	if ((dt < 0) && (retry_cnt == MAX_RECHECK_CNT))
		last_v = v = (last_v + 26);
	else
		last_v = v;

	spin_unlock_irqrestore(&clocksource_read_lock, flags);

	if (retry_cnt == MAX_RECHECK_CNT)
		pr_info("%s:F-updated old:0x%08lx, new:0x%08lx, start:0x%08lx, end:0x%08lx, diff:%lu\n",
			__func__, old_v, last_v,
