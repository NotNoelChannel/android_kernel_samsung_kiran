...
#ifdef CONFIG_SEC_LOG_BUF_NOCACHE
static void __init __reserve_end_of_ram()
{
	int i;
	int totalram = 0;
	uint32_t start;
	for(i = meminfo.nr_banks; i > 0; i--)
		totalram += meminfo.bank[i-1].size;
	start = CONFIG_PHYS_OFFSET+totalram-PAGE_SIZE;
	pr_err("reserve end of ram : %x size : %x", start, totalram);
	memblock_reserve(CONFIG_PHYS_OFFSET+totalram-PAGE_SIZE, PAGE_SIZE);
}
#endif
...
void __init sci_reserve(void)
{
	...
	if(sec_log_buf_nocache_enable==1)
        {
                unsigned int size = sec_logbuf_size+PAGE_SIZE;
                ret = memblock_reserve(sec_logbuf_base_addr, (phys_addr_t)size);
                if( ret != 0 )
                        pr_err("Fail to remove memory for nocache sec log buf\n");
        }
	__reserve_end_of_ram();
#endif
}
