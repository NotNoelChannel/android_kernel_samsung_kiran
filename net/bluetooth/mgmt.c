...
static const u16 mgmt_commands[] = {
	...
	MGMT_OP_SET_RSSI_ENABLE,
	MGMT_OP_GET_RAW_RSSI,
	MGMT_OP_SET_RSSI_DISABLE,
	MGMT_OP_START_LE_DISCOVERY,
	MGMT_OP_STOP_LE_DISCOVERY,
	MGMT_OP_DISABLE_LE_AUTO_CONNECT,
	MGMT_OP_LE_CONN_UPDATE,
	MGMT_OP_SET_MANUFACTURER_DATA,
	MGMT_OP_SET_VOICE_SETTING,
#endif
};
...
static const u16 mgmt_events[] = {
	...
	MGMT_EV_NEW_IRK,
	MGMT_EV_NEW_CSRK,
	MGMT_EV_DEVICE_NAME_UPDATE,
	MGMT_EV_HARDWARE_ERROR,
	MGMT_EV_TX_TIMEOUT_ERROR,
	MGMT_EV_RSSI_ALERT,
	MGMT_EV_RAW_RSSI,
	MGMT_EV_RSSI_ENABLED,
	MGMT_EV_RSSI_DISABLED,
	MGMT_EV_CONN_UPDATED,
	MGMT_EV_CONN_UPDATE_FAILED,
#ifdef CONFIG_TIZEN_RANDOM
	MGMT_EV_NEW_LOCAL_IRK,
#endif
};
		       ...
		       u16 len)
{
	...
	int err;
	__u8 irk[16];
	struct mgmt_ev_new_local_irk ev;
	...
	if (cp->privacy != DISABLE_PRIVACY && cp->privacy != ENABLE_PRIVACY
	...
	*/
	...
	if (cp->privacy == ENABLE_PRIVACY) {
		changed = !test_and_set_bit(HCI_PRIVACY, &hdev->dev_flags);
		memcpy(hdev->irk, cp->irk, sizeof(hdev->irk));
		set_bit(HCI_RPA_EXPIRED, &hdev->dev_flags);
	} else if (cp->privacy == DISABLE_PRIVACY) {
		changed = test_and_clear_bit(HCI_PRIVACY, &hdev->dev_flags);
		memset(hdev->irk, 0, sizeof(hdev->irk));
		clear_bit(HCI_RPA_EXPIRED, &hdev->dev_flags);
	} else {
		if (!smp_generate_irk(hdev->tfm_aes, irk)) {
			memcpy(hdev->irk, irk, sizeof(hdev->irk));
			memcpy(ev.irk, irk, sizeof(ev.irk));
			changed = !test_and_set_bit(HCI_PRIVACY, &hdev->dev_flags);
			set_bit(HCI_RPA_EXPIRED, &hdev->dev_flags);
			mgmt_event(MGMT_EV_NEW_LOCAL_IRK, hdev, &ev,
				sizeof(ev), NULL);
		} else {
			err = cmd_status(sk, hdev->id, MGMT_OP_SET_PRIVACY,
					MGMT_STATUS_FAILED);
			goto unlock;
		}
	}
...
static void set_rssi_threshold_complete(struct hci_dev *hdev, u8 status)
{
	BT_DBG("status 0x%02x", status);
}

static void set_rssi_disable_complete(struct hci_dev *hdev, u8 status)
{
	BT_DBG("status 0x%02x", status);
}
...
static void get_raw_rssi_complete(struct hci_dev *hdev, u8 status)
{
	struct pending_cmd *cmd;
			...
			u8 link_type, u8 addr_type, u8 status)
{
	...
	return mgmt_event(MGMT_EV_CONN_UPDATE_FAILED, hdev, &ev,
				sizeof(ev), NULL);
}
