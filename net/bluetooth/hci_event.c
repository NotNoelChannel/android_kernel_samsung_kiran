...
static void hci_cc_le_remote_conn_param_reply(struct hci_dev *hdev,
						struct sk_buff *skb)
{
	struct hci_rp_le_remote_conn_param_reply *rp = (void *) skb->data;

	BT_DBG("%s status 0x%2.2x", hdev->name, rp->status);
}

static void hci_cc_le_remote_conn_param_neg_reply(struct hci_dev *hdev,
						struct sk_buff *skb)
{
	struct hci_rp_le_remote_conn_param_neg_reply *rp = (void *) skb->data;
...
static void hci_cs_le_conn_update(struct hci_dev *hdev, __u8 status)
{
	struct hci_cp_le_conn_update *cp;

	BT_DBG("%s status 0x%2.2x", hdev->name, status);

	cp = hci_sent_cmd_data(hdev, HCI_OP_LE_CONN_UPDATE);
	if (!cp)
		return;

	if (status) {
		/* command has not being executed by the local controller
		 * some error occured ; need to handle here
		 */
		BT_DBG("%s status 0x%2.2x", hdev->name, status);
	}
}

static void hci_cs_le_remote_features(struct hci_dev *hdev, __u8 status)
{
	BT_DBG("%s status 0x%2.2x", hdev->name, status);
}
...
static void hci_conn_request_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if ((mask & HCI_LM_ACCEPT) &&
		...
		if ((ev->link_type == SCO_LINK || ev->link_type == ESCO_LINK) &&
			...
			cp.reason = HCI_ERROR_REJ_LOW_RESOURCES;
...
static void hci_cmd_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case HCI_OP_LE_REMOTE_CONN_PARAM_REPLY:
		hci_cc_le_remote_conn_param_reply(hdev, skb);
		break;

	case HCI_OP_LE_REMOTE_CONN_PARAM_NEG_REPLY:
		hci_cc_le_remote_conn_param_neg_reply(hdev, skb);
		break;
...
static void hci_cmd_status_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case HCI_OP_LE_CONN_UPDATE:
		hci_cs_le_conn_update(hdev, ev->status);
		break;

	case HCI_OP_LE_REMOTE_FEATURES:
		hci_cs_le_remote_features(hdev, ev->status);
		break;
...
static void hci_le_conn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!conn) {
		...
		l2cap_conn_update_id_addr(conn);
	}

/* Add this piece of code just for the sake of code synchronization
 *  between task_bt_4.1 and w_bt_4.1 branches;
 *  so Keep it commented as no support of Link Layer topology now.
 */
#if 0
	/* Get LE remote features */
	if (conn->type == LE_LINK) {
		struct hci_cp_le_remote_features cp;
		cp.handle = ev->handle;
		hci_send_cmd(hdev, HCI_OP_LE_REMOTE_FEATURES,
				sizeof(cp), &cp);
	}
#endif
...
static void hci_le_remote_features_complete_evt(struct hci_dev *hdev,
						struct sk_buff *skb)
{
	struct hci_ev_le_remote_features_complete *ev = (void *) skb->data;
	struct hci_conn *conn;
	...
	if (!ev->status) {
		memcpy(conn->le_features, ev->le_features, 8);
	}
							...
							struct sk_buff *skb)
{
	...
	/* On a Slave, if no connection parameters are updated, then this
	 * event shall not be issued. On a Master, this event shall be
	 * issued if the Connection_Update commnand was sent.
	 */
	hci_dev_lock(hdev);
	...
	mgmt_le_conn_updated(hdev, &conn->dst, conn->type,
				conn->dst_type, ev->conn_interval,
				ev->conn_latency, ev->supervision_timeout);

unlock:
	mgmt_le_conn_update_failed(hdev, &conn->dst, conn->type,
					  conn->dst_type, ev->status);
	hci_dev_unlock(hdev);
}

static void hci_le_remote_conn_param_request_evt(struct hci_dev *hdev,
							struct sk_buff *skb)
{
	struct hci_ev_le_remote_conn_param *ev = (void *) skb->data;
	struct hci_cp_le_remote_conn_param_reply cp;
	struct hci_cp_le_remote_conn_param_neg_reply neg;
	struct hci_conn *conn;
	...
	hci_send_cmd(hdev, HCI_OP_LE_REMOTE_CONN_PARAM_REPLY, sizeof(cp), &cp);
...
not_found:
	neg.handle = ev->handle;
	neg.reason = HCI_ERROR_UNSUPPORTED_FEATURE_OR_PARAM;
	hci_send_cmd(hdev, HCI_OP_LE_REMOTE_CONN_PARAM_NEG_REPLY,
						sizeof(neg), &neg);
	hci_dev_unlock(hdev);
}
...
static void hci_le_meta_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	case HCI_EV_LE_REMOTE_FEATURES_COMPLETE:
		hci_le_remote_features_complete_evt(hdev, skb);
		break;
	...
	case HCI_EV_LE_REMOTE_CONN_PARAM:
		hci_le_remote_conn_param_request_evt(hdev, skb);
		break;
