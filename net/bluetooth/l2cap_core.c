...
int l2cap_conn_send_param_update_req(struct l2cap_conn *conn,
			u16 interval_min, u16 interval_max,
			u16 latency, u16 time_out)
{
	struct hci_conn *hcon = conn->hcon;
	struct hci_dev *hdev = conn->hcon->hdev;
	...
	/* This command shall only be sent from the LE slave device to
	 * the LE master device and only if one or more of the LE slave
	 * Controller, the LE master Controller, the LE slave Host and
	 * the LE master Host do not support the Connection Parameters
	 * Request Link Layer Control Procedure ([Vol. 6] Part B,
	 * Section 5.1.7).
	 */

	if ((LL_CONN_PARAM & hcon->le_features[0]) &&
					(LL_CONN_PARAM & hdev->le_features[0]))
		return -EINVAL;

	memset(&req, 0, sizeof(req));
	req.min = __cpu_to_le16(interval_min);
	req.max = __cpu_to_le16(interval_max);
	req.latency = __cpu_to_le16(latency);
	req.to_multiplier = __cpu_to_le16(time_out);

	BT_DBG("min 0x%4.4x max 0x%4.4x latency: 0x%4.4x Timeout: 0x%4.4x",
			req.min, req.max, req.latency, req.to_multiplier);
	ident = l2cap_get_ident(conn);
	l2cap_send_cmd(conn, ident, L2CAP_CONN_PARAM_UPDATE_REQ,
						sizeof(req), &req);
	return 0;
}
